...
const fetchCityCoordinates = async (cityId: string): Promise<Coordinates> => {
...
const fetchBasicForecast = async (cityId: string): Promise<ForecastItem[]> => {
  ...
  return data.list.map((item: WeatherApiItem) => ({
...
const CityPage = () => {
  ...
  const [currentData, setCurrentData] = useState<Coordinates | null>(null);
